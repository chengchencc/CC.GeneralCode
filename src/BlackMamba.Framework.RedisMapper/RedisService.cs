...
namespace BlackMamba.Framework.RedisMapper
{
    public class RedisService : IRedisService, IDisposable
    {
        ...
        public long GetSubModelCount<TModel>(string modelId)
        {
            using (var Redis = RedisClientManager.GetReadOnlyClient())
            {
        ...
        public long GetLengthOfQueue<T>(string queueId)
        {
            using (var Redis = RedisClientManager.GetReadOnlyClient())
            {
        ...
        public long RemoveItemFromQueue<T>(string queueId, T queueItem)
        {
            using (var Redis = RedisClientManager.GetClient())
            {
        ...
        public long IncrementValueInHash(string hashKey, string key, int incrementBy = 1)
        {
            using (var Redis = RedisClientManager.GetClient())
            {
