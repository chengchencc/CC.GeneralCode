...
namespace BlackMamba.Framework.RedisMapper
{
    public interface IRedisService
    {
        ...
        long GetSubModelCount<TModel>(string modelId);
        ...
        long RemoveItemFromQueue<T>(string queueId, T queueItem);
        long GetLengthOfQueue<T>(string queueId);

        bool IsAvailable(int connectionTimeoutMillesecs = 200);
        long GetNextSequenceNum(string key);
        long IncrementValueInHash(string hashKey, string key, int incrementBy = 1);
