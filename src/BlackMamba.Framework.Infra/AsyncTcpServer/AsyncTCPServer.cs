...
using System.Net;
namespace BlackMamba.Framework.Infra.AsyncTcpServer
{
    /// <summary>
    ...
    public class AsyncTCPServer : IDisposable
    {
        ...
        /// 获取已连接的客户端
        /// </summary>
        public List<TCPClientState> ConnectedClient {
            get {
                return _clients;
            } }
        ...
        {
            var state = ((KeyValuePair<TcpClient, byte[]>)ar.AsyncState);
            state.Key.GetStream().EndWrite(ar);
            RaiseCompletedSend(state.Key,state.Value);
        }
        #endregion
        ...
        /// 数据发送完毕事件
        /// </summary>
        public event EventHandler<byte[]> CompletedSend;

        /// <summary>
        /// 触发数据发送完毕的事件
        /// </summary>
        /// <param name="state"></param>
        private void RaiseCompletedSend(TcpClient state,byte[] buffer)
        {
            if (CompletedSend != null)
            {
                CompletedSend(state, buffer);
            }
        }
